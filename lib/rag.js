...
// ---------------------------------------------------------------------------
// PDF text extraction using Gemini (gemini-3.5-flash).
// Unlike plain-text extraction (pdf-parse), Gemini reads the PDF with vision,
// so it can transcribe TABLES, structured layouts, and math notation that a
// flat text extractor turns into unreadable gibberish. It runs as a normal
// API call (no native canvas, no worker file, no disk writes), so it is fully
...
async function extractText(fileBuffer) {
    ...
    contents: [
      {
        ...
        parts: [
          ...
          {
            text:
              "Transcribe ALL content from this PDF as clean text. " +
              "For any tables, write out every row and value clearly so the " +
              "data is preserved. Keep question numbers and structure. " +
  ...
  if (!text || text.trim().length === 0) {
    throw new Error("Gemini returned no text from the PDF");
  }

  // pdf-parse used to give us a page count; Gemini doesn't, so we estimate
  // pages from text length purely for the UI's "processed N pages" message.
